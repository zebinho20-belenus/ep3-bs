...
- Unpaid bookings auto-removed by MySQL event `remove_unpaid_bookings` (every 5 min, > 30 min old, `directpay=true`, `status_billing=pending`, **and no reservation before today**). Grace period configurable via `bs_options` `service.payment.unpaid-grace-minutes` — the event itself is recreated by a migration, so changing the option alone does not change the deletion time. Past-dated unpaid bookings are deliberately kept (the court cannot be freed retroactively; the record is needed to collect the money) and surface via `diagnose.php` → `payment.stuck-pending`.
- **MariaDB needs `--event-scheduler=ON`** (`docker-compose.yml`, `docker-compose.dev-server.yml`) — the default is `OFF`, in which case both events exist but never fire. Check: `SHOW VARIABLES LIKE 'event_scheduler'`, or `Event Scheduler: Loaded 2 events` in the MariaDB log. Diagnose check `payment.cleanup-event` reports it as CRITICAL.
