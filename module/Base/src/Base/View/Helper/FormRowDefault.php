...
class FormRowDefault extends AbstractHelper
{
    ...
    public function __invoke($form, $id)
    {
        ...
        if ($type !== 'hidden') {
            ...
            if (strpos($existingClass, 'form-control') === false && strpos($existingClass, 'form-select') === false) {
                if ($formElement instanceof \Zend\Form\Element\Select) {
                    $formElement->setAttribute('class', trim($existingClass . ' form-select'));
                } elseif ($formElement instanceof \Zend\Form\Element\Textarea) {
                    $formElement->setAttribute('class', trim($existingClass . ' form-control'));
        ...
        // Add form-label class to the label element
        ...
        if (strpos($labelClass, 'form-label') === false) {
            $labelAttributes['class'] = trim($labelClass . ' form-label');
