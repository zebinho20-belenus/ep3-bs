...
- **My bookings smart-sort & notification badge (#65, #71)**: Notification badge on "My bookings" (orange = unpaid, green = upcoming). Badge click opens a popover with next 4 bookings summary — desktop only (touch devices: badge is indicator only, no tap conflict). Smart default filter on bookings page: auto-selects "Pending" if unpaid future bookings exist, "Upcoming" if future bookings exist, else "All". Bookings sorted nearest-first (ASC).
